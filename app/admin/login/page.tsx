  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (data.role !== "ADMIN" && data.role !== "SUPERVISOR") {
        setError("This account doesn't have admin access. Use the worker login instead.");
        await fetch("/api/auth/logout", { method: "POST" }); // don't leave a mismatched session set
        return;
      }
  ...
  return (
    <main className="min-h-screen bg-void text-paper flex items-center justify-center px-6">
      <div className="max-w-sm w-full">
        ...
        <h1 className="display text-2xl font-bold mb-1 text-center">Admin Login</h1>
        <p className="text-mist text-sm text-center mb-8">Sign in to view compliance and training data.</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs mono text-mist block mb-1">Admin code</label>
            <input
              value={employeeCode}
